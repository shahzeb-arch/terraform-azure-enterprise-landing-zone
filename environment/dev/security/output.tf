...
output "disk_encryption_set_ids" {
  description = "Disk encryption set IDs."
  value       = { for k, m in module.disk_encryption_set : k => m.id }
}

output "key_vault_key_ids" {
  description = "Key Vault key IDs."
  value       = { for k, m in module.key_vault_key : k => m.id }
}

output "key_vault_key_versionless_ids" {
  description = "Key Vault key versionless IDs (for CMK references)."
  value       = { for k, m in module.key_vault_key : k => m.versionless_id }
}
